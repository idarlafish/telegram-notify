...
export const ReminderFormSchema = v.pipe(
  v.object({
    ...
    repeat: v.picklist(["daily","weekdays","weekends","custom","one_time"]),
    customDays: v.optional(v.array(WeekDaySchema)),
  ...
  v.check((d) => d.repeat !== "custom"   || (d.customDays?.length ?? 0) > 0, "pick at least one day"),
